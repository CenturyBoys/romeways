...
class Spawner(ISpawner):
    ...
    async def start(self):
        if self._region_map.spawn_process:
            await self._process_worker(
                region_map=self._region_map, itineraries=self._itineraries
            )
        else:
    ...
    async def _async_worker(self, region_map: RegionMap, itineraries: List[Itinerary]):
        task = asyncio.create_task(
            ChauffeurService.run(region_map=region_map, itineraries=itineraries)
        )
        self._tasks.append(task)

    async def _process_worker(
        self, region_map: RegionMap, itineraries: List[Itinerary]
    ):
        def process_wrapper(_region_map, _itineraries):
            asyncio.run(
                ChauffeurService.run(region_map=_region_map, itineraries=_itineraries)
            )
